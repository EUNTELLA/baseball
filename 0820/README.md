...
`동적 기준확률 + 잔차 학습 + 3개 연도 순방향 검증`을 공식 데이터만으로 구현합니다.
...
하나의 모델을 시드만 늘리지 않고, 서로 다른 입력 범위와 시간 감쇠를 가진 잔차 채널 3개를 먼저 만듭니다. 이 단계는 채널 구조 선별이므로 각 잔차 채널은 단일시드이며, 통과한 채널만 이후 3시드·6시드로 확대합니다.
...
참조 OOF를 제출 예측에 섞지 않고, 동일한 공식 Train 행에서 우리 기준 OOF보다 오차가 작은 조건만 찾는다. target과 `pitcher_id`가 완전히 일치하지 않으면 즉시 중단한다.

```bash
python 0820/06_external_oof_error_condition_audit_colab.py \
  --train /content/dataset/data/train.csv \
  --external-root /content/reference_oof_assets \
  --output /content/drive/MyDrive/0820_external_oof_error_condition_audit.json \
  --task-type GPU
```

결과에서 2023·2024 모두 양수인 조건만 다음 독립 피처 실험 후보로 사용한다. 참조 예측값·계수·모델은 ZIP에 포함하지 않는다.

## 카운트·손 조합·LI 잔차 차등 선별

OOF 진단에서 반복된 조건 구조만 공식 Train으로 검증한다. 직전 시즌의 조건별 잔차에서 시즌 전체 잔차를 뺀 차등을 축소하여 다음 시즌에 적용한다.
...
조건별 고정표 대신 과거 OOF 잔차를 작은 행 단위 CatBoost가 학습한다. 원시 잔차와 시즌 평균 제거 잔차를 모두 검증한다.
...
Trackman·시즌 이력 파일이 공식 데이터에 포함되어 있는지 파일명, 컬럼과 연결 식별자를 확인한다.
...
결과는 모든 강도에서 2023 음수·2024 양수였다. 가장 약한 0.025도 2023 `-3.44`, 2024 `+13.93`이었고, 0.075는 `-12.08 / +39.69`, 0.15는 `-29.46 / +73.02`였다. 강도에 따라 반대 방향 효과가 함께 커져 안정적인 R 잔차 전이로 볼 수 없으므로 제출 빌드는 만들지 않는다.
