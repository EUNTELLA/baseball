...
여러 결과물을 비교할 때는 모델마다 다른 데이터 분할이나 지표를 사용하면 안 됩니다.

| 비교 항목 | 고정해야 하는 조건 |
| --- | --- |
| 데이터 | 동일한 학습·검증 행 |
| 시간 Fold | 동일한 2023·2024 검증 구간 |
| Feature | 모델 효과를 비교할 때는 동일하게 유지 |
| 지표 | 공식 지표와 동일한 계산 코드 |
| 전처리 | 각 Fold의 학습 데이터로만 적합 |
| 난수 | 재현 가능한 Random seed |

현재 파이프라인은 다음 두 결과물을 생성합니다.

- `fold_results.csv`: 모델별 2023·2024 Log Loss, Brier Score, ROC-AUC
- `leaderboard.csv`: Fold 평균과 변동성, 최종 순위
- `model_comparison.png`: 평균 성능, 시즌별 Log Loss, 보조 지표 차트

평균 점수만 보지 말고 시즌별 점수와 표준편차도 확인해야 합니다. 평균이 조금 좋더라도 특정 시즌에서 크게 무너지거나 변동성이 크다면 Hidden Test에서 불안정할 수 있습니다.

새 모델을 추가할 때는 다음 순서로 판단합니다.

1. 전체 평균 모델보다 좋은가?
2. Logistic Regression보다 좋은가?
3. 2023과 2024에서 모두 개선되는가?
4. 성능 향상이 데이터 누수 때문은 아닌가?
5. 확률 보정과 시즌별 안정성이 유지되는가?
6. 복잡도 증가를 감수할 만큼 개선 폭이 충분한가?
